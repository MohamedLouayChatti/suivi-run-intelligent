...
_REJECTED_COLUMNS: dict[str, str] = {
	"application": "the application is chosen when the file is uploaded, not per row",
	"id": "ticket identifiers are generated by the import",
	"identifiant": "ticket identifiers are generated by the import",
	"date de mise à jour": "this is maintained by the ticket itself",
	"date d'archivage": "archival is not part of an import",
}
...
def resolve_columns(headers: tuple[str, ...]) -> ResolvedColumns:
	...
	for header in headers:
		...
		if column is None:
			...
			if reason is not None:
				resolved.problems.append(f"Column {header!r} is not accepted: {reason}.")
			else:
				resolved.problems.append(f"Unknown column {header!r}.")
			continue
		if column in claimed:
			...
			resolved.problems.append(
				f"Columns {claimed[column]!r} and {header!r} both name {column!r}."
			)
			continue
	...
	if missing:
		resolved.problems.append(f"Missing required column(s): {', '.join(repr(c) for c in missing)}.")
