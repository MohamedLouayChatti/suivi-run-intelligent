...
def get_grant_permission_to_role_handler(uow: Annotated[SqlAlchemyUnitOfWork, Depends(get_unit_of_work)], publisher: Annotated[EventPublisher, Depends(get_event_publisher)], service: Annotated[AuthorizationService, Depends(get_authorization_service)]) -> GrantPermissionToRoleHandler: return _command_handler(GrantPermissionToRoleHandler, uow, publisher, service)
def get_revoke_permission_from_role_handler(uow: Annotated[SqlAlchemyUnitOfWork, Depends(get_unit_of_work)], publisher: Annotated[EventPublisher, Depends(get_event_publisher)], service: Annotated[AuthorizationService, Depends(get_authorization_service)]) -> RevokePermissionFromRoleHandler: return _command_handler(RevokePermissionFromRoleHandler, uow, publisher, service)
