...
class StartProgressHandler:
	...
	async def handle(self, command: StartProgressCommand) -> TicketDetailDTO:
		...
		await self.event_publisher.publish(TicketStatusChanged(ticket.id, old, ticket.status, command.started_at))
