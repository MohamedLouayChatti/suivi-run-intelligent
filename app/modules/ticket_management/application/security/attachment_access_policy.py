...
from app.modules.ticket_management.application.security.support import TicketReadRepositoryScope, parse_uuid
...
class AttachmentAccessPolicy(InstanceAuthorizationPolicy):
	...
	async def authorize(self, *, current_user: CurrentUser, resource_id: Any, operation: str) -> AuthorizationResult:
		...
		# it is the parent ticket_id (no attachment exists yet); for "delete" it
		# is the attachment_id itself.
		if operation == "create":
