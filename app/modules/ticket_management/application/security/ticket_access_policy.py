...
_ASSIGNEE_OR_MANAGER_OPERATIONS = frozenset({"reassign", "change_priority", "archive", "restore", "update_jira", "update_operational_highlight"})


class TicketAccessPolicy(InstanceAuthorizationPolicy):
	"""Instance rules for one ticket.

	Two distinct axes, each backed by its own breadth permission rather than by the caller's
	role: *reach* (is this ticket within my applications, or do I hold
	`ticket.read_any_application`) and *ownership* (am I the assignee, or does a permission
	widen that -- `ticket.manage_any` everywhere, `ticket.manage_primary_application` within
	the application I run).  The operations in `_ASSIGNEE_ONLY_OPERATIONS` are workflow
	...
	async def authorize(self, *, current_user: CurrentUser, resource_id: Any, operation: str) -> AuthorizationResult:
		...
		if operation in _ASSIGNEE_OR_MANAGER_OPERATIONS:
			if ticket.assignee_id == current_user.id or may_manage_others_tickets(current_user, ticket.application):
				return AuthorizationResult(True, "")
			return AuthorizationResult(False, "Only the ticket assignee, or a user allowed to manage this application's tickets, can perform this operation.")
